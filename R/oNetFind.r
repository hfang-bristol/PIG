...
oNetFind <- function(g, scores)
{
    ...
    if(length(pos.nodes)==0){
        ...
        subgraph <- oNetInduce(ig, pos.nodes, knn=0, remove.loops=T, largest.comp=T)
        V(subgraph)$type <- 'desired'
        
    }else{
        
        pos.subgraph <- oNetInduce(ig, pos.nodes, knn=0, remove.loops=T, largest.comp=F)
        ...
        if(length(mst.cluster.id)==1){
            ...
            sub.mig <- oNetInduce(mig, V(mst)[all.ids]$name, knn=0, remove.loops=T, largest.comp=F)
        ...
        if (length(neg.node.ids.2) == 0) {
            ...
            subgraph <- oNetInduce(ig, tmp_nodes, knn=0, remove.loops=F, largest.comp=T)
