...
#' df_SNP <- oSNPscores(data=data, significance.threshold=5e-5, placeholder=placeholder)
#' ## include LD SNPs (calculated based on European populations)
#' df_SNP <- oSNPscores(data=data, significance.threshold=5e-5, include.LD="EUR", placeholder=placeholder)
#' }

oSNPscores <- function(data, include.LD=NA, LD.customised=NULL, LD.r2=0.8, significance.threshold=5e-5, score.cap=10, verbose=TRUE, placeholder=NULL, guid=NULL)
{
	...
	if(length(include.LD) > 0 & is.null(LD.customised)){
		...
		GWAS_LD <- oRDS('GWAS_LD', placeholder=placeholder, guid=guid, verbose=verbose)
		...
		if(1){
			ImmunoBase_LD <- oRDS('ImmunoBase_LD', placeholder=placeholder, guid=guid, verbose=verbose) %>% tibble::deframe()
			res_list <- lapply(include.LD, function(x){
